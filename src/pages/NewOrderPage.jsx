...
function StepPagamento({ form, set, errors, isManut }) {
  ...
  const autoM = form.payment_methods.length > 1 && form.payment_methods.filter(m => parseVal(pd[m].value) === 0).length === 1
    ? form.payment_methods.find(m => parseVal(pd[m].value) === 0) : null

  const getEff = (m) => {
    if (m === autoM) {
      const sum = form.payment_methods.filter(x => x !== m).reduce((s, x) => s + parseVal(pd[x].value), 0)
      ...
      {form.payment_methods.length > 0 && (
        <div style={{ border:`1px solid ${T.ink5}`, borderRadius:12, overflow:'hidden' }}>
          {form.payment_methods.map((m, idx) => {
            ...
            const isLast = idx === form.payment_methods.length - 1
            return (
                ...
                <div style={{ padding:'6px 16px 12px', display:'grid', gridTemplateColumns: m === 'cartao_credito' ? '1fr auto' : '1fr', gap:10, alignItems:'end' }}>
                  <div style={{ border:`1px solid ${T.ink5}`, borderRadius:8, background: isAuto ? T.greenL : T.white, display:'flex', alignItems:'center' }}>
                    ...
                    <input
                      ...
                      readOnly={isAuto}
                      placeholder={form.payment_methods.length === 1 ? fmtNum(total) : '0,00'}
                      style={{
