...
/* ── Warranty status ──────────────────────────────────────────── */
function warrantyStatus(order) {
  if (!order.warranty_months || order.warranty_months === 0) return null
  const start   = new Date(order.created_at)
  const expiry  = new Date(start)
  expiry.setMonth(expiry.getMonth() + parseInt(order.warranty_months))
  const now     = new Date()
  const daysLeft = Math.ceil((expiry - now) / 86400000)

  if (daysLeft < 0)   return { type:'expired',  label:`Garantia vencida há ${Math.abs(daysLeft)} dias`,  bg:'#FEF2F2', text:'#B91C1C', icon:'🔴' }
  if (daysLeft <= 30) return { type:'expiring', label:`Vencendo em ${daysLeft} dia${daysLeft!==1?'s':''}`, bg:'#FFFBEB', text:'#B45309', icon:'⚠️' }
  return { type:'ok', label:`Garantia até ${fmtDate(expiry)}`, bg:'#F0FDF4', text:'#15803D', icon:'🛡️' }
}
  ...
  return (
    <div style={{ padding:'20px 22px 22px', background:'#F7F7F8', borderTop:'1px solid rgba(0,0,0,0.07)' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
        ...
        <button onClick={onClose} style={{ background:'rgba(0,0,0,0.06)', border:'none', borderRadius:6, padding:'4px 10px', cursor:'pointer', fontSize:12, color:'#6B7280', fontFamily:'Instrument Sans,sans-serif' }}>
          Cancelar
        </button>
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          ...
          <div>
            {lbl('E-mail')}
            <input value={form.email} onChange={e=>set('email',e.target.value)} type="text" inputMode="email" autoComplete="email" autoCorrect="off" autoCapitalize="none" spellCheck="false" style={inp('email')} placeholder="cliente@email.com"/>
          </div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr auto', gap:10, alignItems:'end' }}>
          ...
          {lookupCEP.isPending && <Loader2 size={15} style={{ color:'#0A66FF', animation:'spin 1s linear infinite', marginBottom:2 }}/>}
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          <div>{lbl('Endereço')}<input value={form.address} onChange={e=>set('address',e.target.value)} style={inp()} placeholder="Rua, número"/></div>
          <div>{lbl('Complemento')}<input value={form.complement} onChange={e=>set('complement',e.target.value)} style={inp()} placeholder="Apto, bloco..."/></div>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 64px', gap:10 }}>
          <div>{lbl('Bairro')}<input value={form.neighborhood} onChange={e=>set('neighborhood',e.target.value)} style={inp()} placeholder="Bairro"/></div>
          <div>{lbl('Cidade')}<input value={form.city} onChange={e=>set('city',e.target.value)} style={inp()} placeholder="Cidade"/></div>
          <div>{lbl('UF')}<input value={form.state} onChange={e=>set('state',e.target.value.toUpperCase())} maxLength={2} style={inp()} placeholder="SP"/></div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder="Visível apenas para a equipe — não aparece em PDFs"
            rows={3}
        ...
        <button onClick={handleSave} disabled={updateClient.isPending}
          style={{ width:'100%', padding:'11px', background:updateClient.isPending?'#6B7280':'#0C0C0E', color:'#fff', border:'none', borderRadius:9, fontSize:13, fontWeight:600, cursor:updateClient.isPending?'default':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:7, fontFamily:'Instrument Sans,sans-serif', marginTop:4, transition:'background .15s' }}>
          {updateClient.isPending ? <><Loader2 size={13} style={{ animation:'spin 1s linear infinite' }}/> Salvando...</> : <><Save size={13}/> Salvar alterações</>}
        </button>
      </div>
    </div>
  )
...
function OrderActions({ order, clientEmail, clientName, docUrl, setDocUrl }) {
  ...
  const handleDownload = async e => {
    ...
    try {
      ...
      const a = document.createElement('a'); a.href=url; a.download=`garantia-${order.order_number}.pdf`; a.click()
...
function OrderCard({ order, clientEmail, clientName, isFirst, isLast, onDocSaved }) {
  ...
  return (
    <div style={{ display:'flex', gap:12, marginBottom:4 }}>
      ...
      <div style={{ flex:1, borderRadius:12, overflow:'hidden', marginBottom:8, border:`1px solid ${wStatus?.type==='expired'?'#FECACA':wStatus?.type==='expiring'?'#FDE68A':'rgba(0,0,0,0.07)'}`, background:'#fff', boxShadow:isFirst?'0 2px 8px rgba(0,0,0,0.07)':'0 1px 3px rgba(0,0,0,0.04)' }}>

        {/* Alerta de garantia — faixa no topo do card */}
        {wStatus && wStatus.type !== 'ok' && (
          <div style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 14px', background:wStatus.bg, borderBottom:`1px solid ${wStatus.type==='expired'?'#FECACA':'#FDE68A'}` }}>
            <AlertTriangle size={11} style={{ color:wStatus.text, flexShrink:0 }}/>
            <span style={{ fontSize:11, fontWeight:600, color:wStatus.text }}>{wStatus.label}</span>
          </div>
        )}
        ...
        <div onClick={()=>setExpanded(e=>!e)}
          ...
          <div style={{ flex:1, minWidth:0 }}>
            ...
            <div style={{ display:'flex', gap:12, marginTop:4, flexWrap:'wrap' }}>
              <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:11, color:'#9CA3AF' }}><Calendar size={10}/>{fmtDate(order.created_at)}</span>
              {payments.length>0 && <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:11, color:'#9CA3AF' }}><CreditCard size={10}/>{payments.map(p=>PAY_LABEL[p]||p).join(' + ')}</span>}
              {order.warranty_months>0 && wStatus?.type==='ok' && (
                <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:11, color:'#16A34A' }}><Shield size={10}/>{order.warranty_months} {order.warranty_months===1?'mês':'meses'} garantia</span>
              )}
            </div>
          </div>
...
function InfoRow({ icon:Icon, label, value, raw, onCopy, copied }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'11px 16px' }}>
      ...
      {!empty && raw && (
        <button onClick={()=>onCopy(raw,label)} style={{ background:'none', border:'none', cursor:'pointer', padding:6, borderRadius:6, color:copied?'#16A34A':'#9CA3AF', transition:'color .15s' }}>
...
export default function ClientHistory({ clientId, onClose }) {
  ...
  const copy = (val, field) => {
    if (!val) return
    navigator.clipboard.writeText(val).then(() => { setCopiedField(field); setTimeout(()=>setCopiedField(null),2000) })
  }
  ...
  const handleNovoAtendimento = () => {
    onClose()
    navigate(`/orders/new?client_id=${clientId}`)
  }
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        {upsell && !isLoading && (
          <div style={{ background:'#EEF4FF', borderBottom:'1px solid #BFDBFE', padding:'9px 20px', display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
            ...
            <span style={{ fontSize:12, color:'#1D4ED8', fontWeight:500 }}>Oportunidade: histórico com {metrics.topModels[0]?.model} — considere oferecer {upsell}</span>
          </div>
        )}
          ...
          {!editing && !isLoading && (
            <section style={{ margin:'16px 20px 0' }}>
              ...
              {client.internal_note && (
                <div style={{ marginTop:12, background:'#FFFBEB', border:'1px solid #FDE68A', borderRadius:10, padding:'12px 14px', display:'flex', gap:10 }}>
                  <FileText size={14} style={{ color:'#B45309', flexShrink:0, marginTop:1 }}/>
                  <div>
                    <div style={{ fontSize:10, fontWeight:700, color:'#B45309', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:4 }}>Nota interna</div>
                    <div style={{ fontSize:13, color:'#92400E', lineHeight:1.6 }}>{client.internal_note}</div>
                  </div>
                </div>
              )}
            </section>
