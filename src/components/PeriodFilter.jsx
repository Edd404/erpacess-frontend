...
// ─────────────────────────────────────────────────────────────
// Filtro de período unificado: pills rápidas (7/30/90d) +
// seletor de mês + calendário de intervalo.
//
// Props:
//   value    { mode: 'quick'|'month'|'range', days?, year?, month?, from?, to? }
//   onChange (value) => void
// ─────────────────────────────────────────────────────────────
...
const C = {
  ...
  popShadow:  '0 8px 32px rgba(0,0,0,0.14), 0 0 0 0.5px rgba(0,0,0,0.08)',
}
  ...
  return (
    <div style={{ userSelect:'none' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <button onClick={onPrevMonth} style={btnNav}>
          <ChevronLeft size={14} />
        </button>
        <span style={{ fontSize:13, fontWeight:700, color:C.text }}>
          {MONTHS_PT[month]} {year}
        </span>
        <button onClick={onNextMonth} style={btnNav}>
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: marked ? 8 : inRng ? 0 : 8,
                borderTopLeftRadius:  (start || (!rangeFrom && inRng)) ? 8 : inRng ? 0 : 8,
                borderBottomLeftRadius:(start || (!rangeFrom && inRng)) ? 8 : inRng ? 0 : 8,
                borderTopRightRadius:  end ? 8 : inRng ? 0 : 8,
                borderBottomRightRadius: end ? 8 : inRng ? 0 : 8,
...
// ── Month Grid (para seletor de mês) ──────────────────────────
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:6 }}>
        {MONTHS_SHORT.map((m, i) => {
          ...
          return (
            <button
              key={m}
              onClick={() => !isFuture && onSelect(year, i)}
              disabled={isFuture}
              style={{
...
// ── Popover wrapper ───────────────────────────────────────────
function Popover({ anchor, open, onClose, children, align = 'right' }) {
  ...
  return (
    <div ref={ref} style={{
      ...
      boxShadow:C.popShadow, padding:16,
      zIndex:500, minWidth:240,
...
export default function PeriodFilter({ value, onChange, align = 'right' }) {
  ...
  // estado interno de qual popover está aberto
  const [openMonth, setOpenMonth] = useState(false)
  const [openCal,   setOpenCal]   = useState(false)

  // para o calendário: estado de navegação e seleção temporária
  const [calYear,  setCalYear]  = useState(now.getFullYear())
  const [calMonth, setCalMonth] = useState(now.getMonth())
  const [picking,  setPicking]  = useState(null)   // primeira data selecionada (esperando segunda)
  const [hovered,  setHovered]  = useState(null)

  // para o mês: ano de navegação do grid
  const [mYear, setMYear] = useState(now.getFullYear())

  const monthBtnRef = useRef(null)
  const calBtnRef   = useRef(null)

  // Inicializa o calendário com as datas do value quando abre
  useEffect(() => {
    if (openCal) {
  ...
  const handleQuick = (days) => {
    onChange({ mode:'quick', days })
    setOpenMonth(false); setOpenCal(false)
  }
  ...
  const handleDayHover = (iso) => {
    if (picking) setHovered(iso)
  }

  const handleClear = (e) => {
    e.stopPropagation()
    onChange({ mode:'quick', days: 30 })
  }
    ...
    <>
      <div style={{ display:'inline-flex', alignItems:'center', gap:6, flexWrap:'wrap' }}>
        ...
        <div style={{ position:'relative' }}>
          ...
          <Popover anchor={monthBtnRef} open={openMonth} onClose={() => setOpenMonth(false)} align={align}>
            <MonthGrid
              year={mYear}
        ...
        <div style={{ position:'relative' }}>
          ...
          <Popover anchor={calBtnRef} open={openCal} onClose={() => { setOpenCal(false); setPicking(null); setHovered(null) }} align={align}>
            {picking && (
              <div style={{
            ...
            {(rangeFrom || picking) && (
              <button
                onClick={() => { setPicking(null); setHovered(null) }}
                style={{
        ...
        {/* Badge de limpar quando month/range ativos */}
...
export function periodToParams(value) {
  ...
  if (value.mode === 'quick') {
    return { period: String(value.days) }
  }
